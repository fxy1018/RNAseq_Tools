...
class DownLoadFileTool(object):
    ...
    def downloadFileCurl(self, File, dLocation):
        cmd = 'sudo curl ' + dLocation + File.fileName + " \"" + File.fileLink + "\" "
        try:
            os.system(cmd)
        except:
            logging.warning(File.fileName+"fails")
...
def crateLinksDocument(file, location):
...
if __name__=="__main__":
    with open("linksFile_07122017.csv", 'rb') as linkFile:
        content = linkFile.readlines()

    content = [x.strip("\n") for x in content]
    count = 0
    f = open("log_07122017.log", "w") 
         
    for line in content:
        downTool = DownLoadFileTool()
        targetFile = downTool.createFile(line)
        dLoc = "/data/cofactor_genomics/" + targetFile.expNum + "/"
        downTool.downloadFileCurl(targetFile, dLoc)
